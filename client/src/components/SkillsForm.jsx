  ...
  return (
    <div className='space-y-4'>
            ...
            <div className='flex gap-2'>
                <input type="text" placeholder='Enter a Skill (e.g., Javascript,python)' className='flex-1 px-3 text-sm ' onChange={(e)=>setNewSkill(e.target.value)}
                value={newSkill}
                ...
                <div className='flex flex-wrap gap-2'>
                    {
                        data.map((skill,index)=>(
                            <span key={index} className=' flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm'>
                                {skill}
                                <button onClick={()=>removeSkill(index)} className='ml-1 hover:bg-blue-200 rounded-full p0.5 transition-colors'>
            ...
            <div className='bg-blue-50 p-3 rounded-lg'>
                <p className='text-sm text-blue-800'><strong>
                    Tip: </strong> Add 8-12 relevant skills. Include both technical skills (programming languages, tools) and soft skills (leadership, communication).</p>
            </div>
    </div>
  )
}
